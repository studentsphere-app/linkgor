...
import { getPlanning, type Lesson } from "../src";
...
function printLessons(lessons: Lesson[]) {
	for (const lesson of lessons) {
		...
		const timeFrom = start.toLocaleTimeString("fr-FR", {
			hour: "2-digit",
			minute: "2-digit",
		});
		const timeTo = end.toLocaleTimeString("fr-FR", {
			hour: "2-digit",
			minute: "2-digit",
		});
		...
		console.log(
			chalk.cyan("┌─ ") +
				chalk.bold.yellow(timeFrom) +
				chalk.cyan(" → ") +
				chalk.bold.yellow(timeTo),
		);
